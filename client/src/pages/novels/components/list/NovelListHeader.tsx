  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="text-2xl font-semibold tracking-normal sm:text-3xl">
                {props.view === "shelf" ? "我的书架" : "小说工作台"}
              </h1>
              <div className="inline-flex rounded-md bg-muted/50 p-1" role="tablist" aria-label="小说列表视图">
                <Button
                  type="button"
                  size="sm"
                  variant={props.view === "shelf" ? "default" : "ghost"}
                  onClick={() => props.onViewChange("shelf")}
                  role="tab"
                  aria-selected={props.view === "shelf"}
                >
                ...
                <Button
                  type="button"
                  size="sm"
                  variant={props.view === "workbench" ? "default" : "ghost"}
                  onClick={() => props.onViewChange("workbench")}
                  role="tab"
                  aria-selected={props.view === "workbench"}
                >
                  <LayoutDashboard className="mr-1.5 h-4 w-4" aria-hidden="true" />
                  工作台
                </Button>
              </div>
            </div>
        ...
        <div className="mobile-full-actions grid w-full gap-2 sm:max-w-md sm:grid-cols-1 lg:max-w-sm">
      ...
      {props.view === "workbench" ? (
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 border-y border-border/60 py-3 text-sm">
          ...
          {props.recoveryCandidateCount > 0 ? (
            <Button type="button" size="sm" variant="ghost" className="h-10 px-2" onClick={props.onOpenRecovery}>
