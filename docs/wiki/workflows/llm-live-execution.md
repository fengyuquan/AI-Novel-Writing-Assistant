...
2. 服务端是流的唯一消费者。浏览器仅订阅事件；页面关闭、切换任务或 SSE 断开不能中断后台任务。用户若要停止生成，必须通过实况面板的「中断」按钮（或对应取消 API）显式取消；该操作会 abort 当前会话绑定的模型流，并把实况标为 `cancelled`。
...
10.1. 「中断」只作用于仍在进行中的 LLM 实况会话：`POST /api/llm-live/sessions/:interactionId/cancel` 中断单次调用，`POST /api/llm-live/cancel-active` 中断当前可见范围内全部进行中调用。中断不会回滚已经写入的业务结果；调用方应按既有错误/取消路径收尾。
