...
- 漫画工作台「粘贴原文」支持保真改编：适合粘贴新闻、报道等纪实文本，分镜与气泡尽量沿用原文引语和事实，再走分话、分格、出图与导出。
- 粘贴原文时可选择「保真改编」或「创意改编」；保真适合新闻，创意适合需要压缩叙述的长文原文。
- 保真分格完成后若检测到与原文不完全一致，会在话列表给出提示，仍可按原计划继续出图，并可在分格页改气泡。
...
- 章节编辑器在保存按钮下方提供「运行完整审校」：可用当前编辑区正文（含未保存修改）随时复查，并直接查看 AI 指出的问题与修改建议。
- 章节编辑器可直接切换上一章/下一章；有未保存修改时离开会提示，避免误丢改稿。
- 侧栏补充前后章衔接、目标字数对照，以及写法与一致性提醒，方便改开头结尾时对照。
- 支持轻审当前选段或开头结尾，审校问题可标为已处理；窄屏改为「引导 / 正文 / AI」页签切换。
- AI 修正结果强化原文与候选对照，并支持 Ctrl/⌘+S 保存、Ctrl/⌘+Enter 发起或接受修正。
- 章节编辑器新增写作辅助：本章任务单、开篇/中段/结尾检查点、卡文续写方向、人设口吻速查、写完自检、完成度进度、专注模式、短时冲刺、审校定位与一键 AI 改、快照时间线，以及朗读与拗口提示。
- 审校问题「定位到正文」更稳：即使证据偏描述，也会按原文摘录、共用片段或开篇/中段/结尾位置推断；新一轮审校也会优先给出可定位的原文短摘录；定位成功时不再弹出提示。
- 正文编辑框顶部可调字号、行距与段距，并提供朗读控件：从光标所在段读到章末，倍速、人声、正在朗读段落高亮；朗读中空格暂停/继续，方向键跳到上一段/下一段。
- 章节编辑器「保存」只快速写入正文；另有「同步保存」才会更新摘要、检索索引等关联资料，并分别显示「已保存 / 待同步 / 已同步」状态。
- 章节编辑器新增「AI 写法检测」：可找出读起来像 AI、模板或说明书的句子，给出修改建议，并支持定位到正文或按建议用 AI 改。
- 章节编辑器右侧「AI 修正」默认收起，可在正文区顶部随时打开；字数、保存/同步状态与 AI 写法检测也集中到正文顶部，方便边写边看。
- 手机上「引导与审校」「AI 修正」页签可正常上下滚动；右上角「AI 创作实况」悬浮按钮可拖动到合适位置。
- 章节编辑器新增「范本对照」：可选多本范本生成对照稿，写好后打开对照窗；按段落左右对齐（每一行是同一段），你的正文和 AI 范文都可直接改；支持铺满全屏、窗内定位、多标签切换，以及最多 4 列并排（默认 1 列）。范文按「范本 + 模型」分版本保存到本章，下次打开同一章仍在。

### 2026-08-08（世界图谱交互与大纲开书）

- 新增「从大纲开书」：在小说列表或手动创建页进入后，粘贴已有章节大纲，系统会抽出书名、简介、角色和世界观草稿供你确认，再一键创建小说并写入拆章。
- 从大纲开书后会尽量带上故事宏观、卷战略与节奏板，开书核对也会展示章节摘要、目标和任务单，方便直接进入「节奏 / 拆章」继续细化。
- 从大纲开书写入的「章节摘要」与「章节目标」会分开保留，不再被互相覆盖；同步变更预览也会按章节目标正确提示。
- 「节奏 / 拆章」支持导入外部章节大纲：粘贴 Markdown 或自由文本，先预览再写入草稿；可整本替换当前拆章后保存，并继续用系统细化和写作。
- 从大纲开书或导入大纲时，若章节只有大纲里的任务单笔记、还没有完整场景拆解，不再误报「执行合同未通过质量门禁」而卡住；可先写入章节清单，再去细化后开写。
- 导入大纲时会对照角色、世界观、卷战略和节奏板做设定冲突检查；可逐条选择跟设定、跟大纲或稍后处理，不会静默改掉你的设定卡。
- 导入优先按模板识别章节；格式较乱时可用 AI 整理成章节清单，不必再完全依赖系统自动拆章。
- 工作台支持页面级查找替换：顶部可打开面板，或使用 Ctrl+H；可在当前页的正文编辑区和各类输入框中查找、逐条替换或一键全部替换。
- 手机站点可在「更多」里打开同一查找替换能力，方便改名、统一用语时不必逐个输入框手工修改。
...
- DeepSeek 默认模型切换为 `deepseek-v4-flash`，设置页可选 `deepseek-v4-pro`；仍保留旧模型名以便兼容已有配置。
- DeepSeek V4 Flash / Pro 支持在设置中开关思考模式，写作和规划调用可按需要启用或关闭。
- 自动导演每一轮会给出五套整书方向；都不满意时可以输入提示词再生成新一轮，网页端与命令行都能这样做。
- 新增命令行交互壳：在终端里用菜单和问答完成创作环境配置、选书/新建、自动导演开书、进度跟踪、继续/批准关卡、章节预览和运行记录查看。
- 命令行支持“修改故事规划”：可查看大纲摘要，用一句话反馈让 AI 完善或重做当前规划步骤，也可优化文字大纲并保存；与网页端共用同一本书和导演任务。
- 命令行新增“大纲优先开书”：可设置期望章数（默认 800）与每章字数下限（默认 3000），先生成章节标题和详细大纲（必写/禁写/其他约束），不写正文；支持导出 Markdown，设置会保存到本机供下次直接选用。
- 命令行与网页端共用同一套后端数据，不改变原有界面；适合想用键盘推进、同时保留网页工作台的用户。
