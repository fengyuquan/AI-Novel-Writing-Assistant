...
import {
  ...
  createDirectorCandidateResponseSchema,
...
export function createDirectorCandidatePrompt(
  count = 2,
): PromptAsset<
  DirectorCandidatePromptInput,
  ReturnType<typeof createDirectorCandidateResponseSchema>["_output"]
> {
  ...
  return {
    ...
    outputSchema: createDirectorCandidateResponseSchema(safeCount),
    render: (input, context) => [
      new SystemMessage([
        ...
        `必须精准输出 ${safeCount} 套候选，数量不得多也不得少。`,
        ...
        `可选字段 titleOptions（最多 ${Math.max(1, Math.min(8, input.titleOptionsMax ?? 4))} 条）：为封面与点击向的书名备选。每条必须含 title、clickRate（35-99 的整数）、style。`,
...
export const directorCandidatePrompt = createDirectorCandidatePrompt(2);

// Keep registry typing stable for default schema export.
void directorCandidateResponseSchema;
...
> = {
  ...
  render: (input, context) => [
    new SystemMessage([
      ...
      `可选字段 titleOptions（最多 ${Math.max(1, Math.min(8, input.titleOptionsMax ?? 4))} 条）：每条含 title、clickRate（35-99）、style；style 只能是 literary、conflict、suspense、high_concept 四者之一（英文小写）。`,
