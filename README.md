...
- 漫画工作室生图确认弹窗可分别选择图片供应商与该供应商下的具体模型；角色三视图、表情稿、格子图、资产图和场景设定图共用这套确认流程，可在生图前改 Prompt、参考图和模型参数。
- 系统设置新增「默认图片模型」；漫画项目页的图片厂商选择会与它对齐，文本类生成仍用顶部模型。
- 漫画生图支持人工干预：可复制提示词到外部工具出图，再粘贴或上传结果；有附加参考图时提示词不加文生图声明，全部取消附加图后会自动补上。
- 一次生成多张候选图时可先预览再选一张落盘，之后还能改选；格子图可放大查看候选。
- 漫画工作台补充分步引导，角色三视图支持清除与本地上传/粘贴。
- 短剧工作台可一键生成切图提示词包，方便复制到外部出图工具。
- 章节编辑器新增「切图提示词」面板，可按本章正文整理出图清单并复制。
- AI 创作实况支持中断当前请求，长时间卡住时可主动停下再试。
- 范本对照窗口补充状态提示与对齐操作说明，方便对照时快速看清当前进度。
