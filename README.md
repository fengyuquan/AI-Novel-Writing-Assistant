...
- 系统设置 → 系统维护中可打开「提示词调用记录」：查看系统发给模型的提示词，支持筛选、导出和清空，便于排查生成结果。
- 默认自动记录请求提示词（不含模型回复），可随时关闭记录或调整保留条数。
- 系统维护新增「人工中继」：开启后，系统调用模型前会弹出窗口，可复制提示词交给外部 API，再把返回内容粘贴回来继续创作。
- 系统设置 → 写作质量增强中可调整「生成数量」：自定义开书灵感、书名候选、角色候选、封面图、导演方向候选套数等默认一次生成多少条。
