...
- 自动导演每一轮给出五套整书方向；不满意时可输入提示词再生成新一轮（网页与命令行均可）。
- 新增命令行交互壳：可在终端用菜单和问答配置模型、选择小说、启动自动导演、跟踪进度并预览章节。
- 命令行可查看并修订故事规划：用一句话告诉 AI 怎么改大纲，确认后继续写书；与网页端共用同一套数据。
- 命令行支持“大纲优先开书”：可设 800 章 / 每章不少于 3000 字等目标，先出标题与详细大纲并导出，设置可保存下次直接用。
- 命令行与网页端共用同一套后端数据，原有界面保持不变；启动方式见 `pnpm dev:cli`。
