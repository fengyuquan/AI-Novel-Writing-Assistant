...
- 新增「从大纲开书」：粘贴已有章节大纲后，可确认书名、简介、角色和世界观草稿，再一键创建小说并写入拆章。
- 从大纲开书后会尽量带上故事宏观、卷战略与节奏板，开书核对也会展示章节摘要、目标和任务单，方便直接进入「节奏 / 拆章」继续细化。
- 从大纲开书写入的「章节摘要」与「章节目标」会分开保留，不再被互相覆盖；同步变更预览也会按章节目标正确提示。
- 「节奏 / 拆章」支持导入外部章节大纲：粘贴后可预览并写入草稿，整本替换当前拆章后再保存，继续用系统细化和写作。
- 从大纲开书或导入大纲时，只有任务单笔记、尚未细化场景的章节不再被误拦；可先落好章节清单，再继续细化开写。
- 导入大纲时会检查与现有设定的冲突，可逐条选择跟设定、跟大纲或稍后处理。
- 导入优先按模板识别章节；格式较乱时可用 AI 整理成章节清单。
- 工作台支持页面级查找替换：顶部可打开面板，或使用 Ctrl+H；可在当前页正文编辑区和各类输入框中查找、逐条替换或一键全部替换。
- 手机站点可在「更多」里打开同一查找替换能力，改名或统一用语时不必逐个输入框手工修改。
...
- 公网自托管可使用预构建 Docker 镜像与 Nginx 访问密码门禁，便于在云主机上只给自己使用。
