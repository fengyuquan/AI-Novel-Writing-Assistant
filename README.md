...
- 漫画分话页可用「分格方式」切换自动 / 人工：人工模式下先预览并复制分格提示词，到外部模型生成后再粘贴 JSON 写回本话格子。
- 漫画项目页顶部可用「出图方式」切换自动 / 人工：人工模式下弹出提示词窗口，复制后自行调用外部 API 出图，再粘贴或上传结果继续后续流程。
- 手机端「更多」里可进入漫画工作台；打开漫画项目后顶部标题会显示「漫画项目」，项目内页签也可左右滑动切换。
- 短剧、角色库、小说封面和生图确认弹窗等单独选模型的入口，都会优先跟随顶部/系统里已选的文本模型与默认图片模型。
- 生图确认与格子列表在窄屏上更紧凑，复制提示词在弹窗与桌面壳里也更稳。
