...
- 从一句灵感直接进入自动导演，无需先手写世界观、主线、角色和卷纲；系统先整理项目设定、对齐书级 framing，再生成五套整本方向和对应标题组
- 方向不满意时可以输入意见再生成下一轮、定向修订某一套方案、或只重做某套方案的标题组，避免"满意就确认 / 不满意就整批重来"
...
- 手机顶部也可切换厂商和模型；可用模型会记住，只有点刷新才会重新拉取。
- 手机上打开小说后，可从顶部直接进入任务进度；自动导演阻塞或异常时会给出明确的继续 / 恢复入口。
- 任务详情在手机上改为底部抽屉，先展示当前下一步，再查看推进记录和排查信息。
- 章节执行改成「正文优先」：章节列表和 AI 辅助收入底部面板，底部主按钮直接写本章、审校或继续处理。
- 首页下一步入口在窄屏下更紧凑，主操作全宽，更容易一点继续当前作品。
- 手机小说工作区会突出当前步骤和流程推荐；卷战略、节奏拆章等专家能力默认收起，需要时再打开。
- 创作中枢在手机上优先展示推荐下一步和对话；小说绑定与创作线程收入底部面板，不抢主任务。
- 手机设置页顶部可直接打开快捷配置，先完成厂商、密钥和模型，再进入完整设置。
- 手机章节编辑器改为正文优先：章节信息和 AI 改写收入底部面板，底部可直接保存。
- 运行记录在手机上先看任务列表，点选后再打开详情抽屉处理恢复与后续动作。
- 手机小说列表优先突出「AI 自动导演开书」，书架改为单列，作品进度说明默认可见。
- 自动导演开书各步骤主按钮在手机上粘性全宽，起始想法页更省纵向滚动。
- 导演跟进在手机上先看列表，点选后再打开详情抽屉；概览分区改为更易读的两列布局。
- 手机首页状态摘要改为两列可读指标，不再挤成四列或隐藏说明。
- 运行记录筛选在手机上改为单列，导演跟进与设置页布局由页面自己控制，减少窄屏被全局样式压扁。
- 手机长页可一键回到顶部或滑到底部，避开底部主操作按钮。
- AI 创作实况图标可拖到任意位置，并记住你上次放的位置；轻点仍可打开实况。
- 小说工作区（含章节执行）顶部新增导航抽屉，可直接回首页、小说列表、运行记录或继续自动导演。
- 底部弹出面板上滑浏览时更稳，触顶后不会轻易整层滑走；章节编辑页也可一键回到正文顶部或底部。
- 手机章节编辑页可打开导航抽屉回首页，并用上一章 / 下一章快速切换。
- 手机章节改写改为「正文对比 + 底部决策」：选中后可一键优化/扩写/精简；生成后直接在正文看红绿对比，底部选用版本。
- 范本对照窗段落错位时，可对本侧使用「空出本段」「并入上段」手动对齐：空出后当前内容跳到下一段，并入则与上一段合并。
- 对照窗里点选范文正文或切换范文标签只高亮当前行，不再同步滚动跳转；需要跳转时仍用点评区的「定位到对应段落」。
- 范本对照会先提炼范文精髓（声口、对话、节奏等可学习要点），再按精髓写对照稿，并显示哪些要点已做到、哪些还缺。
- 较长章节会分段仿写后再统一语气；若精髓要点未覆盖，可一键按缺口再生成。
- 手机打开范本对照时默认全屏：竖屏按段堆叠（先你的正文再范文），矮横屏恢复并排；顶栏/底栏可收成箭头，并可用段号导航弱段。
- 手机章节编辑在正文未改动时不再常驻「已是最新」悬浮保存条；有未保存修改时才出现「保存本章」。
- 打开范本对照时会盖住背后的抽屉、底栏和其他浮层，减少对照页里透出其它页面内容。
- 知识库可直接上传更大的 `.txt` 整本文档；云部署需同步放宽接口与网关体积上限后生效。
- 书架支持「封面网格 / 紧凑列表」两种排列，选择会记住；也可直接上传封面图，与 AI 生成封面共用同一图库。
- 新增独立的数据管理控制台：按小说浏览章节、角色与大纲，必要时也可做表级维护；需配置管理口令后使用。
- DeepSeek 默认模型切换为 `deepseek-v4-flash`，设置页也可直接选择 `deepseek-v4-pro`；旧模型名仍可继续使用。
- DeepSeek V4 Flash / Pro 支持按需开关思考模式，方便按写作任务调节速度与深度。
...
如果你想用命令行菜单和问答推进创作（与网页端共用同一套数据，不改原界面），先保证后端已启动，再执行：

```bash
pnpm dev:cli
```

更多说明见 [cli/README.md](./cli/README.md)。
